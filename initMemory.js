...
module.exports.initMemory = function() {
	utils.cL(JSON.stringify(Memory));
	if (!Memory.init) {
		...
		for (var name in Game.spawns) {
			...
			// Get third Source location
			var secondFiltered = _.pull(filteredIds, secondSource);
			var thirdSource = mainSpawn.pos.findClosest(secondFiltered);
			mainSpawn.memory.thirdSourceId = thirdSource.id;
		...
		// Set up Counters: Each role type, each source
